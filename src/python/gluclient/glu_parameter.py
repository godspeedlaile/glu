...
class GluParameter(object):
    """
    Represents a parameter for a Glu resource or service.
...
"""GluParameter '%s':
    ...
    def __init__(self, name, pdef):
        ...
        try:
            ...
            if self.__type_str not in self.__TYPE_CONVERT:
                raise GluClientException(("Server error: Type '%s' specified for parameter '%s', which is " +
            ...
            if self.__DEFAULT_KEY in pdef:
                ...
                if not self.__required:
                    raise GluClientException("Server error: No default value specified for optional parameter '%s'." % self.__name)

        except KeyError, e:
            raise GluClientException("Server error: Expected key '%s' missing in definition of parameter '%s'." % (str(e), self.__name))
    ...
    def sanity_check(self, value):
        ...
        if type(value) not in suitable_types:
            raise GluClientException("Type '%s' is not suitable for parameter '%s'. Has to be one of '%s'." % (type(value), self.__name, suitable_types))
